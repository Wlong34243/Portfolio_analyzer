        ...
        try:
            ...
            summary_data = {
                ...
                'Value': [
                    ...
                    f"{metrics['single_largest_weight']:.1f}%",
                    f"{metrics['top_5_concentration']:.1f}%",
                    f"{metrics['top_10_concentration']:.1f}%",
                    f"{metrics['cash_percentage']:.1f}%",
                    f"{len(agg_df['Account'].unique())}",
            ...
            consolidated_export = symbol_agg[['Symbol', 'Description', 'Market_Value', 'Consolidated_Weight', 
                                            'Quantity', 'Day_Change_Dollar', 'Sector', 'Account_Count', 'Account']].copy()
            consolidated_export.columns = ['Symbol', 'Description', 'Market_Value', 'Weight_Pct', 
                                          'Total_Quantity', 'Day_Change_Dollar', 'Sector', 'Account_Count', 'Accounts']
            
            individual_export = agg_df[['Symbol', 'Description', 'Account', 'Market_Value', 'Position_Weight',
                                      'Quantity', 'Price', 'Day_Change_Dollar', 'Day_Change_Percent', 'Sector']].copy()
            individual_export.columns = ['Symbol', 'Description', 'Account', 'Market_Value', 'Weight_Pct',
                                       'Quantity', 'Price', 'Day_Change_Dollar', 'Day_Change_Percent', 'Sector']
            individual_export = individual_export.sort_values('Market_Value', ascending=False)
            
            # Sector and account summaries
            sector_summary = agg_df.groupby('Sector').agg({
                'Market_Value': 'sum',
                'Symbol': 'count',
                'Day_Change_Dollar': 'sum'
            }).round(2)
            sector_summary['Weight_Pct'] = (sector_summary['Market_Value'] / metrics['total_portfolio_value'] * 100).round(1)
            sector_summary['Avg_Position_Size'] = (sector_summary['Market_Value'] / sector_summary['Symbol']).round(2)
            sector_summary.columns = ['Total_Value', 'Position_Count', 'Day_Change_Dollar', 'Weight_Pct', 'Avg_Position_Size']
            sector_summary = sector_summary.sort_values('Total_Value', ascending=False).reset_index()
            
            account_summary = agg_df.groupby('Account').agg({
                'Market_Value': 'sum',
                'Symbol': 'count',
                'Day_Change_Dollar': 'sum'
            }).round(2)
            account_summary['Weight_Pct'] = (account_summary['Market_Value'] / metrics['total_portfolio_value'] * 100).round(1)
            account_summary['Avg_Position_Size'] = (account_summary['Market_Value'] / account_summary['Symbol']).round(2)
            account_summary.columns = ['Total_Value', 'Position_Count', 'Day_Change_Dollar', 'Weight_Pct', 'Avg_Position_Size']
            account_summary = account_summary.sort_values('Total_Value', ascending=False).reset_index()
            
            # Handle empty dataframes for Excel export
            if tech_df.empty:
                tech_df = pd.DataFrame([{
                    'Symbol': 'N/A',
                    'Current_Price': 0,
                    'RSI': 0,
                    'Signal': 'No individual stocks in top positions',
                    'Note': 'Consider analyzing individual stock positions'
                }])
            
            if options_df.empty:
                options_df = pd.DataFrame([{
                    'Symbol': 'N/A',
                    'Strategy': 'No Analysis',
                    'Monthly_Yield_Est': 0,
                    'Note': f'No positions above {options_min_weight}% threshold'
                }])
            
            # Create Excel file
            excel_data = create_excel_report(
                summary_df, consolidated_export, individual_export, 
                sector_summary, account_summary, tech_df, options_df, risk_df
            )
            ...
            st.download_button(
                ...
                help="Download comprehensive portfolio analysis with all 8 sheets"
            )
            ...
            <div class="success-box">
                ...
                <ul>
                    <li>📋 <strong>Portfolio Summary</strong> - Key metrics dashboard</li>
                    <li>🎯 <strong>Consolidated Positions</strong> - True position sizes across accounts</li>
                    <li>📊 <strong>Individual Positions</strong> - All positions by account</li>
                    <li>🏭 <strong>Sector Analysis</strong> - Sector allocation and performance</li>
                    <li>🏦 <strong>Account Analysis</strong> - Account breakdown</li>
                    <li>📈 <strong>Technical Analysis</strong> - Buy/sell signals and indicators</li>
                    <li>💼 <strong>Options Strategies</strong> - Covered call opportunities</li>
                    <li>⚠️ <strong>Risk Analysis</strong> - Concentration warnings and recommendations</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
